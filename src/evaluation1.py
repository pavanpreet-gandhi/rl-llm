...
def evaluate(
    ...
    # Running totals for summary metrics
    running_num_episodes = 0
    running_success_sum = 0
    running_reward_sum = 0
    running_length_sum = 0
    running_invalid_sum = 0
    running_success_lengths = []  # Lengths of successful episodes for avg_steps_to_success
    ...
    for batch_idx in range(num_batches):
        ...
        running_avg_reward = running_reward_sum / num_episodes_so_far if num_episodes_so_far > 0 else 0
        running_avg_length = running_length_sum / num_episodes_so_far if num_episodes_so_far > 0 else 0
        running_avg_invalid = running_invalid_sum / num_episodes_so_far if num_episodes_so_far > 0 else 0
        running_avg_steps_to_success = (
            sum(running_success_lengths) / len(running_success_lengths) if running_success_lengths else float("nan")
        )
        running_avg_total_time = sum(total_times) / len(total_times) if total_times else 0
        running_avg_generate_time = sum(total_generate_times) / len(total_generate_times) if total_generate_times else 0

        # Log per-batch and running metrics to wandb
        if log_to_wandb:
            batch_step = step_offset + batch_idx
            wandb.log({
                # Per-batch metrics (already time series)
                ...
                # Running versions of summary metrics (now time series)
                f"eval/{model_name}/{env_id}/ctx{context_window}/num_episodes": running_num_episodes,
                f"eval/{model_name}/{env_id}/ctx{context_window}/success_rate": running_success_rate,
                f"eval/{model_name}/{env_id}/ctx{context_window}/avg_reward": running_avg_reward,
                f"eval/{model_name}/{env_id}/ctx{context_window}/avg_steps": running_avg_length,
                f"eval/{model_name}/{env_id}/ctx{context_window}/invalid_action_rate": running_avg_invalid,
                f"eval/{model_name}/{env_id}/ctx{context_window}/avg_steps_to_success": running_avg_steps_to_success,
                f"eval/{model_name}/{env_id}/ctx{context_window}/avg_total_time": running_avg_total_time,
                f"eval/{model_name}/{env_id}/ctx{context_window}/avg_generate_time": running_avg_generate_time,
            }, step=batch_step)
    ...
    # Log final summary metrics as a table and in wandb.summary
    if log_to_wandb:
        # Create a table for final summary metrics
        table = wandb.Table(columns=["Metric", "Value"])
        for metric_name, value in metrics.items():
            table.add_data(metric_name, value)
            # Also log to wandb.summary
            wandb.summary[f"eval/{model_name}/{env_id}/ctx{context_window}/{metric_name}"] = value
        wandb.log({f"eval/{model_name}/{env_id}/ctx{context_window}/summary_table": table}, step=step_offset + num_batches)
